...
def run_point_cloud_registration(movable_cloud: str,
    ...
    inds0 = np.random.choice(np.asarray(pcd_move.points).shape[0], patches_per_pair, replace=False)
    inds1 = np.random.choice(np.asarray(pcd_stay.points).shape[0], patches_per_pair, replace=False)
...
def run_point_cloud_registration_o3d(pcd_move: o3d.geometry.PointCloud,
