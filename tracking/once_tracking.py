...
def get_next_string(input_string):
    try:
    ...
    number += 1
    next_string = f"{number:06d}"
    return next_string

def build_sequence_id_to_index(pickle_data):
    sequence_id_to_index = {}
    for index, data in enumerate(pickle_data):
        sequence_id = data.get('sequence_id', None)
        if sequence_id is not None and sequence_id not in sequence_id_to_index:
            sequence_id_to_index[sequence_id] = index
    return sequence_id_to_index
...
def track_instances(dataset, dataset_root, seq_id, pickle_data, seq_id_to_index, outfile):
    ...
    while i < (len_frames - 1):
        ...
        frame_id = pickle_data[i].get('frame_id', None)
        next_frame_id = pickle_data[next_i].get('frame_id', None)
        ...
        if not instances_dict:
            ...
            pickle_data[i]["annos"]["instance_ids"] = first_instance_ids
        ...
        pickle_data[next_i]["annos"]["instance_ids"] = next_instance_ids

        i += 1

    try:
        with open(outpath, 'wb') as destination_file:
            pickle.dump(pickle_data, destination_file)
...
if __name__ == '__main__':
    ...
    seq_id_to_index = build_sequence_id_to_index(pickle_data)
    # parallel_process(scenes, dataset, dataset_root)
    outfile = 'once_raw_small_track'
    track_instances(dataset, dataset_root, scenes[0], pickle_data, seq_id_to_index, outfile)  # for one scene!!!
